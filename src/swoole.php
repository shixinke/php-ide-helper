<?php
define('SWOOLE_BASE', 4);               //使用BASE模式
define('SWOOLE_THREAD', 2);             //使用线程模式
define('SWOOLE_PROCESS', 3);            //使用进程模式
...
class swoole_server
{
     ...
     * @param int $mode：运行模式(共有SWOOLE_BASE基本、SWOOLE_PROCESS多进程、SWOOLE_THREAD三种模式)
     * @param int $socket_type:socket类型(支持TCP/UDP、TCP6/UDP6、UnixSock Stream/Dgram 6种)
     */
    public function __construct(string $host, int $port, int $mode = SWOOLE_PROCESS, int $socket_type = SWOOLE_SOCK_TCP)
    {
     ...
     */
    public function addlistener(string $host, int $port, $socket_type = SWOOLE_SOCK_TCP)
    {
     ...
     * @param int $socket_type：监听的socket类型
     */
    public function listen(string $host, int $port, $socket_type = SWOOLE_SOCK_TCP)
    {
...
class swoole_table implements Iterator
{
     ...
     */
    public function incr(string $key, string $column, mixed $incrby = 1)
    {
     ...
     */
    public function decr(string $key, string $column, mixed $decrby = 1)
    {
...
class swoole_http_server extends swoole_server
{
     ...
     * 注：不建议使用
     */
    public function setGlobal(int $flags = HTTP_GLOBAL_ALL, int $flags = HTTP_GLOBAL_GET | HTTP_GLOBAL_POST)
    {
