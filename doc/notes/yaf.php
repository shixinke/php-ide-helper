<?php
/**
 * Yaf 一个使用C开发的轻量级的PHP框架，包含基本的MVC，路由等功能
 */
 ...
 * @package Yaf
 */
final class YafX_Application
{
    /**
     * (Yaf >= 3.0.2)
     * 全局配置实例
     *
     * 根据实例化YafX_Application时传入的ini配置文件路径或者配置数组及配置节点名称，实例化的YafX_Config_Ini或者YafX_Config_Simple对象.
     *
     * PHP代码可以这样获取：
     *
     * $config = YafX_Application::app()->getConfig();
     *
     * @var YafX_Config_Abstract
     */
    protected $config;
     ...
     *
     * PHP代码可以这样获取：
     * $dispatcher = YafX_Application::app()->getDispatcher();
     *
     * @var YafX_Dispatcher
     */
     ...
     *
     * PHP代码可以这样获取：
     * $app = YafX_Application::app();
     *
     * @var YafX_Application
     */
     ...
     *
     * PHP代码可以这样获取：
     * $modules = YafX_Application::app()->geModules();
     *
     * @var String
     */
    protected $_modules;
     ...
     *
     * PHP代码可以这样获取：
     * $environ = YafX_Application::app()->environ();
     *
     * @var String
     */
    protected $_environ;
     ...
     *
     * PHP代码可以这样获取：
     * $err_on = YafX_Application::app()->getLastErrorNo();
     *
     * @var Int
     */
    protected $_err_no;
     ...
     *
     * PHP代码可以这样获取：
     * $err_msg = YafX_Application::app()->getLastErrorMsg();
     *
     * @var String
     */
    protected $_err_msg;
     ...
     * 调用bootstrap
     *
     * 指示YafX_Application去寻找Bootstrap，并按照声明的顺序，执行所有在Bootstrap类中定义的以_init开头的方法.（php.net文档有误）.
     *
     * @return YafX_Application
     */
 ...
 *
 * Bootstrap是用来在YafX_Application运行(run)之前做一些初始化工作的机制.
 * 你可以通过继承YafX_Bootstrap_Abstract来定义自己的Bootstrap类.
 * 在Bootstrap类中所有以"_init"开头的公有的方法, 都会被按照定义顺序依次在YafX_Application::bootstrap()被调用的时刻调用.
 *
 * @package Yaf
 */
abstract class YafX_Bootstrap_Abstract{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Dispatcher
 *
 * YafX_Dispatcher实现了MVC中的C分发，它由YafX_Application负责初始化，然后由YafX_Application::run启动，它协调路由来的请求，并分发和执行发现的动作.
 * 并收集动作产生的响应，输出响应给请求者，并在整个过程完成以后返回响应.
 *
 * @package Yaf
 */
final class YafX_Dispatcher
{
    /**
     * (Yaf >= 3.0.2)
     * 当前请求对象（包含请求的所有信息）.
     *
     * PHP代码可以这样获取：
     * $request = YafX_Dispatcher::getInstance()->getRequest();
     *
     * @var YafX_Request_Abstract
     */
    protected $_request;
     ...
     *
     * PHP代码可以这样设置并初始化视图对象：
     * YafX_Dispatcher::getInstance()->setView($view);
     * YafX_Dispatcher::getInstance()->initView($template_dir, $option);
     *
     * @var YafX_View_Interface
     */
    protected $_view;
     ...
     *
     * PHP代码可以这样获取：
     * $router = YafX_Dispatcher::getInstance()->getRouter();
     *
     * @var YafX_Router
     */
    protected $_router;
     ...
     *
     * PHP代码可以这样获取：
     * $dispatcher = YafX_Dispatcher::getInstance();
     *
     * @var YafX_Dispatcher
     */
     ...
     *
     * PHP代码可以这样设置：
     * YafX_Dispatcher::getInstance()->autoRender($flag);
     *
     * @var Boolean
     */
     ...
     *
     * PHP代码可以这样设置：
     * YafX_Dispatcher::getInstance()->setDefaultModule($module);
     *
     * @var String
     */
     ...
     *
     * PHP代码可以这样设置：
     * YafX_Dispatcher::getInstance()->setDefaultController($controller);
     *
     * @var String
     */
     ...
     *
     * PHP代码可以这样设置：
     * YafX_Dispatcher::getInstance()->setDefaultAction($action);
     *
     * @var String
     */
     ...
     *
     * PHP代码可以这样注册插件：
     * YafX_Dispatcher::getInstance()::registerPlugin(new ModuleInitPlugin());
     *
     * @var Array
     */
    protected $_plugins;
     ...
     * 开启/关闭异常抛出或返回当前状态
     *
     * 当传递$flag参数时，设置抛出异常，并返回YafX_Dispatcher
     * 当不传递任何参数时，返回抛出异常状态
     *
     * @param boolean $flag
     *
     ...
     *
     * 当传递$flag参数时，设置自动异常捕获，并返回YafX_Dispatcher
     * 当不传递任何参数时，返回当前状态
     *
     * 注意：如果开启了YafX_Dispatcher::catchException() （可以通过设置application.dispatcher.catchException来开启），
 ...
 *
 * YafX_Config_Abstract被设计在应用程序中简化访问和使用配置数据.它为在应用程序代码中访问这样的配置数据提供了一个基于用户接口的嵌入式对象属性.
 * 配置数据可能来自于各种支持等级结构数据存储的媒体.YafX_Config_Abstract实现了Countable，ArrayAccess和Iterator接口.
 * 这样，可以基于YafX_Config_Abstract对象使用count()函数和PHP语句如foreach，也可以通过数组方式访问YafX_Config_Abstract的元素.
 *
 * @package Yaf
 */
abstract class YafX_Config_Abstract implements Iterator, Countable, ArrayAccess
{
    /**
     * (Yaf >= 3.0.2)
     * 存储已解析的配置
     *
     * PHP代码可以这样获取：
     * $config = YafX_Application::app()->getConfig()->toArray();
     *
     * @var array
     */
    protected $_config;
 ...
 *
 * YafX_Controller_Abstract是Yaf的MVC体系的核心部分.
 * MVC是指Model-View-Controller，是一个用于分离应用逻辑和表现逻辑的设计模式.
 * YafX_Controller_Abstract体系具有可扩展性，可以通过继承已有的类，来实现这个抽象类，从而添加应用自己的应用逻辑.
 * 对于Controller来说, 真正的执行体是在Controller中定义的一个一个的动作, 当然这些动作也可以定义在Controller外.
 *
 * @package Yaf
 */
abstract class YafX_Controller_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 动作名与动作类文件路径映射数组
     ...
     * 每个动作都需要实现YafX_Action_Abstract就可以通过定义YafX_Controller_Abstract::$actions来指明
     * 那些动作对应于具体的那些分离的类.
     *
     * @var array
     */
     ...
     * 初始化视图对象
     *
     * @deprecated 一直不可用，调用此方法只会返回YafX_Controller_Abstract的实例
     *
     * @return YafX_Controller_Abstract
     */
     ...
     *
     * 调用YafX_Controller_Abstract::forward()以后，不会直接立即跳转到目的Action执行，
 ...
 *
 * YafX_Action_Abstract是MVC中C的动作，一般而言动作都是定义在YafX_Controller_Abstract的派生类中的.
 * 但是有的时候，为了使得代码清晰，分离一些大的控制器，则可以采用单独定义YafX_Action_Abstract来实现.
 *
 * @package Yaf
 */
abstract class YafX_Action_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 当前请求的控制器实例
     *
     * @var YafX_Controller_Abstract
     */
...
/**
 * (Yaf >= 3.0.2)
 * Interface YafX_View_Interface
 *
 * YafX_View_Interface是为了提供可扩展的，可自定的视图引擎而设立的视图引擎接口，它定义了用在Yaf上的视图引擎需要实现的方法和功能.
 *
 * @package Yaf
 */
interface YafX_View_Interface
 ...
 * Class YafX_Loader
 *
 * YafX_Loader类为Yaf提供了自动加载功能，它根据类名中包含的路径信息实现类的定位和自动加载.
 *
 * @package Yaf
 */
final class YafX_Loader
{
    /**
     * (Yaf >= 3.0.2)
     * 当前应用本地类库目录
     ...
     *
     * PHP可以这样获取：
     * $global_library = YafX_Loader::getInstance()->getLibraryPath(true);
     *
     * @var String
     */
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Plugin_Abstract
 *
 * YafX_Plugin_Abstract是Yaf的插件基类，所有应用在Yaf的插件都需要继承实现这个类，这个类定义了7个方法，依次在7个时机的时候被调用.
 *
 * @package Yaf
 */
abstract class YafX_Plugin_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 在路由之前触发
 ...
 * 我们可以通过YafX_Registry类的静态方法来使用对象注册表.
 * 另外，由于该类是一个数组对象，你可以使用数组形式来访问其中的类方法.
 *
 * @package Yaf
 */
final class YafX_Registry
{
    /**
     * (Yaf >= 3.0.2)
     * Registry实例（单例模式）
 ...
 *
 * 代表了一个实际请求，一般的不用自己实例化它，YafX_Application在run以后会自动根据当前请求实例它.
 *
 * @package Yaf
 */
abstract class YafX_Request_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 当前请求的模块名
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Response_Abstract
 *
 * 响应对象和请求对象相对应, 是发送给请求端的响应的载体
 *
 * @package Yaf
 */
abstract class YafX_Response_Abstract
{
...
/**
 * (Yaf >= 3.0.2)
 * Interface YafX_Route_Interface
 *
...
interface YafX_Route_Interface
{
     ...
     * @return Boolean
     */
    public function route(YafX_Request_Abstract $request);
 ...
 * @package Yaf
 */
class YafX_Route_Static implements YafX_Route_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 匹配请求
...
final class YafX_Router
{
     ...
     * @return YafX_Router
     */
    public function addRoute($name, YafX_Route_Interface $route){}
 ...
 * @package Yaf
 */
final class YafX_Session implements Iterator, ArrayAccess, Countable
{
    /**
     * (Yaf >= 3.0.2)
     * 重置__construct魔术方法.
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception
 *
 * Yaf异常基类
 *
 * @package Yaf
 */
class YafX_Exception extends Exception
{
    /**
     * (Yaf >= 3.0.2)
     * 异常信息
 ...
 * @package Yaf\Config
 */
final class YafX_Config_Ini extends YafX_Config_Abstract
{
    /**
     * (Yaf >= 3.0.2)
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Config_Simple
 *
 * Simple为存储在PHP的数组中的配置数据提供了适配器
 *
 * @package Yaf\Config
 */
final class YafX_Config_Simple extends YafX_Config_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 构造方法，初始化YafX_Config_Simple对象
     *
 ...
 *
 * 代表了一个实际的Http请求，一般的不用自己实例化它，YafX_Application在run以后会自动根据当前请求实例它.
 *
 * @package Yaf\Request
 */
class YafX_Request_Http extends YafX_Request_Abstract
{
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Request_Simple
 *
 * 代表了一个实际的请求，一般的不用自己实例化它，YafX_Application在run以后会自动根据当前请求实例它.
 *
 * @package Yaf\Request
 */
class YafX_Request_Simple extends YafX_Request_Abstract
{
    const SCHEME_HTTP  = 'http';
    const SCHEME_HTTPS = 'https';
 ...
 *
 * YafX_View_Simple是Yaf自带的视图引擎，对于视图模板，就是普通的PHP脚本.
 *
 * @package Yaf\View
 */
class YafX_View_Simple implements YafX_View_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 储存所有模板变量
 ...
 *
 * YafX_Response_Cli是在Yaf作为命令行应用的时候默认响应载体
 *
 * @package Yaf\Response
 */
class YafX_Response_Cli extends YafX_Response_Abstract{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Response_Http
 *
 * Http是在Yaf作为Web应用的时候默认响应载体
 *
 * @package Yaf\Response
 */
class YafX_Response_Http extends YafX_Response_Abstract
{
    /**
     * (Yaf >= 3.0.2)
     * 是否开启已输出响应报头检测
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Route_Map
 *
 * Map议是一种简单的路由协议, 它将REQUEST_URI中以'/'分割的节, 组合在一起, 形成一个分层的控制器或者动作的路由结果.
 *
 * @package Yaf\Route
 */
final class YafX_Route_Map implements YafX_Route_Interface
{
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Route_Regex
 *
 * 正则路由协议
 *
 * @package Yaf\Route
 */
final class YafX_Route_Regex implements YafX_Route_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 匹配模式（正则表达式）
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Route_Rewrite
 *
 * 重写路由协议
 *
 * @package Yaf\Route
 */
final class YafX_Route_Rewrite implements YafX_Route_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 匹配模式（正则表达式）
 ...
 * @package Yaf\Route
 */
final class YafX_Route_Simple implements YafX_Route_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 模块名
 ...
 * @package Yaf\Route
 */
final class YafX_Route_Supervar implements YafX_Route_Interface
{
    /**
     * (Yaf >= 3.0.2)
     * 全局路由变量名
...
/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_StartupError
 *
 * Yaf App实例启动异常
 *
 * @package Yaf\Exception
 */
class YafX_Exception_StartupError extends YafX_Exception{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_RouterFailed
 *
 * 路由异常
 *
 * @package Yaf\Exception
 */
class YafX_Exception_RouterFailed extends YafX_Exception{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_DispatchFailed
 *
 * 分发异常
 *
 * @package Yaf\Exception
 */
class YafX_Exception_DispatchFailed extends YafX_Exception{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_LoadFailed
 *
 * 装载异常
 *
 * @package Yaf\Exception
 */
class YafX_Exception_LoadFailed extends YafX_Exception{}
 ...
 * @package Yaf\Exception
 */
class YafX_Exception_TypeError extends YafX_Exception{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_LoadFailed_Module
 *
 * 模块加载异常
 *
 * @package Yaf\Exception\LoadFailed
 */
class YafX_Exception_LoadFailed_Module extends YafX_Exception_LoadFailed{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_LoadFailed_Controller
 *
 * 控制器加载异常
 *
 * @package Yaf\Exception\LoadFailed
 */
class YafX_Exception_LoadFailed_Controller extends YafX_Exception_LoadFailed{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_LoadFailed_Action
 *
 * 动作类加载异常
 *
 * @package Yaf\Exception\LoadFailed
 */
class YafX_Exception_LoadFailed_Action extends YafX_Exception_LoadFailed{}

/**
 * (Yaf >= 3.0.2)
 * Class YafX_Exception_LoadFailed_View
 *
 * 视图加载异常
 *
 * @package Yaf\Exception\LoadFailed
 */
class YafX_Exception_LoadFailed_View extends YafX_Exception_LoadFailed{}